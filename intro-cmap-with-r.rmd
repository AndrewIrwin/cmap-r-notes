...
Here are a bunch of examples; I'll only execute the last one.
...
Averaging over time is a bit trickier. You need to do a computation with the time variable. For example, here we will compute the year (as an integer) of the time variable and add to that the day of year divided by 365 to get a decimal year. (Which will sometimes be wrong because of leap years.)
...
Better still is to count the number of days since a reference day.
...
mq = glue("select min([time]) as start_time, max([time]) as end_time,
                  ...
                  where [time] between '2016-01-01' and '2016-12-31' and
                                           lat between 40 and 50 and
                                           lon between 50 and 60 
                  group by round((lat-0.5)/2,0)*2+0.5, round((lon-0.5)/2,0)*2+0.5, round(datediff(day, '2016-01-01', [time])/15.0, 0)*15.0
                  having count(chl) > 0
                  order by mean_lat, mean_lon
            ")
a <- exec_manualquery(mq)
...
CMAP comes with several functions to extract and aggregate data including `get_section`, 
`get_depthprofile`, `get_spacetime`, and `get_timeseries`. 

Since the data are in relational database you can query with SQL, its possible to create your own queries. Here I will explore several kinds of query you can write.
...
* basic select statements and the modifiers top(), distinct
* changing the name of selected variables
* summary functions count(), average(), stdev(), and how to count missing data
...
Now we know what the variables are called. Write a more query to subset the data. Time is specified as an integer month (unlike most data in Darwin that has an ISO date and time.)
...
Compute chl slope / growth rate and plot distributions.
...
tblWOA_Climatology - no time variable? missing. are there months somewhere?
tblCHL_REP 1998-2018
tblDarwin_Climatology 1993-2015

Get phyto and zoo data from Darwin. Time series at a few points.


```{r}
search_metadata("darwin", long_name_only = FALSE) %>% group_by(Temporal_Resolution) %>% count()
search_metadata("darwin", long_name_only = FALSE) %>% filter(Temporal_Resolution == "Three Days")
```

```{r}
table_info("tblDarwin_Phytoplankton")
```

(These metadata leave a bit to be desired. Where is the model and parameter inforamation?)

Most of the Darwin output tables are climatologies. We have about 20 years of data for a model with 5 functional groups.

```{r}
tbl = "tblDarwin_Phytoplankton"
get_head(tbl)
```

```{r}
lat_c = 40.25 # centre and delta
lat_d = 0.2
lon_c = -55.25
lon_d = 0.2 # 0.5 degree resolution
depth_min = 0
depth_max = 10
mq = glue("select count(*) as n
                  from {tbl} where 
                  abs(lat - {lat_c}) <= {lat_d} and
                  abs(lon - {lon_c}) <= {lon_d} and
                  depth between {depth_min} and {depth_max}
...
use a grid from one table to look up values in another table (e.g., momthly 1x1 WOA to get chl from satellite, averaged over 20 years)


compute global mean chl time series
N & S hemisphere
zonally averaged
...
percentiles of abundance, log abundance, log relative abundance
frequency table of species, sorted by total abundance, mean abundance, median abundance
cropped to show the top few
grouped by day or study site


Challenge exercise
Find MLD using density profiles from ARGO
